...
tPlaylist *procuraPlaylist(tListaPlaylist *sent, char *nome)
{
    ...
    while (1)
    {
        if (p == NULL)
            return NULL;
        else if (strcmp(nome, retornaNomePlay(p->playList)) == 0)
            return p->playList;
        else
            p = p->prox;
    }
...
void printaListaplayList(tListaPlaylist *sent, FILE *arq, char *nomeUsu)
{
    ...
    for (p = sent->pri; p != NULL; p = p->prox)
    {
        ...
        printaListaMusica(sentMus, arqPlay);
        fclose(arqPlay);
    }
    fprintf(arq, "\n");
}
